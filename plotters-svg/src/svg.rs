...
/// to call [open_tag](AttrWriter::open_tag), then zero or more times calling [write_key](AttrWriter::write_key),
/// and finally calling one of [close](AttrWriter::close) (to close a self-closing tag) or [finish_without_closing](AttrWriter::finish_without_closing)
// (to schedule writing the closing tag for later).
...
impl<'a> DrawingBackend for SVGBackend<'a> {
    ...
    fn blit_bitmap<'b>(
        ...
        let mut attrwriter = self.open_tag_2();
